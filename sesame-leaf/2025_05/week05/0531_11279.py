...
def max_Heap_add(heap_list:list[int], add_num:int) -> None:
...
def max_Heap_pop(heap_list:list[int]) -> int:
...
def main() -> None:
    ...
    for _ in range(operation_num):
        ...
        if input_num == 0:
            result.append(max_Heap_pop(heap_list))
        else:
            max_Heap_add(heap_list, input_num)
